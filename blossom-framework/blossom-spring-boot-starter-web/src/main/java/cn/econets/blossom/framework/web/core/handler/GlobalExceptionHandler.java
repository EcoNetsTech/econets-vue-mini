...
public class GlobalExceptionHandler {
    ...
    private CommonResult<?> handleTableNotExists(Throwable ex) {
        ...
        if (message.contains("report_")) {
            log.error("[报表模块 blossom-module-report - 表结构未导入]");
            return CommonResult.error(GlobalErrorCodeConstants.NOT_IMPLEMENTED.getCode(),
                    "[报表模块 blossom-module-report - 表结构未导入]");
        }
        // 2. 工作流
        if (message.contains("bpm_")) {
            log.error("[工作流模块 blossom-module-bpm - 表结构未导入]");
            return CommonResult.error(GlobalErrorCodeConstants.NOT_IMPLEMENTED.getCode(),
                    "[工作流模块 blossom-module-bpm - 表结构未导入]");
        }
        // 3. 微信公众号
        if (message.contains("mp_")) {
            log.error("[微信公众号 blossom-module-mp - 表结构未导入]");
            return CommonResult.error(GlobalErrorCodeConstants.NOT_IMPLEMENTED.getCode(),
                    "[微信公众号 blossom-module-mp - 表结构未导入]");
        }
        // 4. 商城系统
